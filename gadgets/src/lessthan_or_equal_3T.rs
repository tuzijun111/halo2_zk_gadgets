...
pub trait LtEq3TInstruction<F: Field> {
...
pub struct LtEq3TConfig<F, const N_BYTES: usize> {
...
impl<F: Field, const N_BYTES: usize> LtEq3TConfig<F, N_BYTES> {
...
#[derive(Clone, Debug)]
pub struct LtEq3TChip<F, const N_BYTES: usize> {
    config: LtEq3TConfig<F, N_BYTES>,
}

impl<F: Field, const N_BYTES: usize> LtEq3TChip<F, N_BYTES> {
    ...
    pub fn configure(
        ...
        lhs2: impl FnOnce(&mut VirtualCells<F>) -> Expression<F>,

        rhs: impl FnOnce(&mut VirtualCells<F>) -> Expression<F>,
        rhs1: impl FnOnce(&mut VirtualCells<F>) -> Expression<F>,
        rhs2: impl FnOnce(&mut VirtualCells<F>) -> Expression<F>,
        
    ) -> LtEq3TConfig<F, N_BYTES> {
        ...
        meta.create_gate("lt gate", |meta| {
            ...
            let con1 = lhs(meta) + lhs1(meta) + lhs2(meta);
            let con2 = rhs(meta) + rhs1(meta) + rhs2(meta);
    ...
    pub fn construct(config: LtEq3TConfig<F, N_BYTES>) -> LtEq3TChip<F, N_BYTES> {
        LtEq3TChip { config }
    }
}

impl<F: Field, const N_BYTES: usize> LtEq3TInstruction<F> for LtEq3TChip<F, N_BYTES> {
    fn assign(
        &self,
        region: &mut Region<'_, F>,
        offset: usize,
        ...
        let con1 = lhs + lhs1 + lhs2;
        let con2 = rhs + rhs1 + rhs2;
...
impl<F: Field, const N_BYTES: usize> Chip<F> for LtEq3TChip<F, N_BYTES> {
    type Config = LtEq3TConfig<F, N_BYTES>;
    type Loaded = ();
